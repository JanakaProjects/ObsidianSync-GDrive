  ...
  async uploadFile(file, force = false, driveMap) {
    ...
    try {
      ...
      if (driveMap && driveMap[file.path]) {
        ...
        const parentFolderId2 = await this.getFolderIdForFile(file.path);
        const query = `name='${file.name}' and '${parentFolderId2}' in parents and trashed=false and mimeType!='application/vnd.google-apps.folder'`;
